  ...
  methods: {
    async addComment() {
      var commentPost = await axios.post(
        `https://www.endorphinoutdoor.com/endorphin/wp-json/wp/v2/comments`,
        {
    ...
    async getComments() {
      var comments = (
        await axios.get(
          `https://www.endorphinoutdoor.com/endorphin/wp-json/wp/v2/comments?post=${this.post.id}`
        )
      ).data;
    ...
    async getRelatedPost() {
      try {
        var relatedPosts = (
          await axios.get(
            `https://www.endorphinoutdoor.com/endorphin/wp-json/wp/v2/posts?cat=${this.categories[0].id}&categories_exclude=46,47,48,49&per_page=4`
          )
        ).data;
      ...
      try {
        ...
        for (const post of relatedPosts) {
          ...
          //   await axios.get(
          //     `https://www.endorphinoutdoor.com/endorphin/wp-json/wp/v2/media/${post.featured_media}`
          //   )
          // ).data.source_url;
    ...
    async getPost() {
      ...
      try {
        if (this.categories.length == 0 && this.tags.length == 0) {
          ...
          var categoriesRaw = (
            await axios.get(
              `https://www.endorphinoutdoor.com/endorphin/wp-json/wp/v2/categories`
            )
          ).data;

          var tagsRaw = (
            await axios.get(
              `https://www.endorphinoutdoor.com/endorphin/wp-json/wp/v2/tags`
            )
          ).data;
